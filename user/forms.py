...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        user = authenticate(username=username_or_email, password=password)  #只能通过authenticate(username,password)判断用户是否存在
        if user is None:
...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名',max_length=30,min_length=5, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入用户名(5-30位)'}))
    ...
    password = forms.CharField(label='密码',
                               min_length=8,
                               widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入密码(最少8位)'}))
...
class ChangePasswordForm(forms.Form):
    ...
    new_password = forms.CharField(
        label='新的密码',
        min_length=8,
        widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入新的密码(最少8位)'})
    )
